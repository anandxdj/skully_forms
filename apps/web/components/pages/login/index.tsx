...
export default function LoginPageView() {
  ...
  // Determine current active illustration based on current view and theme
  const getIllustrationSrc = () => {
    const isDark = theme === "dark";
    if (currentView === "check-email" || currentView === "forgot") {
      return isDark ? ASSETS.illustrations.emptyBoxSkeletonDark : ASSETS.illustrations.emptyBoxSkeletonLight;
    }
    return isDark ? ASSETS.illustrations.developerSkeletonDark : ASSETS.illustrations.developerSkeletonLight;
  };

  return (
    <div className="flex min-h-screen bg-background text-foreground transition-colors duration-300 font-sans relative overflow-hidden select-none">
      
      {/* Visual Organic Wavy Mesh Backgrounds */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden select-none">
        <div className="absolute top-[10%] left-[-10%] w-[480px] h-[480px] rounded-full bg-[#ff2e8c]/5 blur-[100px]" />
        <div className="absolute bottom-[10%] right-[-10%] w-[580px] h-[580px] rounded-full bg-primary/5 blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 flex flex-col justify-between py-6">
        ...
        <main className="my-auto py-12 flex items-center justify-center">
          <div className="w-full max-w-5xl rounded-[32px] border border-border/80 bg-card/65 backdrop-blur-md shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[580px]">
                ...
                <div className="space-y-6 flex-1 flex flex-col justify-center">
                  ...
                  {currentView === "forgot" && (
                    <form onSubmit={handleForgotSubmit} className="space-y-6 animate-fade-in">
                      <div className="space-y-1.5">
                        <h2 className="text-2xl font-black tracking-tight text-foreground font-sans">Forgot password?</h2>
                  ...
                  {currentView === "check-email" && (
                    <div className="space-y-6 animate-fade-in text-center lg:text-left">
                      ...
                      <div className="bg-[#10b981]/5 border border-[#10b981]/15 p-4.5 rounded-2xl flex items-start gap-3">
            ...
            {/* 2. RIGHT COLUMN: Visual Premium Illustration Panel (6 columns) */}
            <div className="lg:col-span-6 bg-muted/20 relative p-8 sm:p-12 flex flex-col justify-center items-center overflow-hidden">
              {/* Backing Gradient Radial Mesh */}
              <div className="absolute top-[20%] right-[-15%] w-[380px] h-[380px] rounded-full bg-[#FAF8F5]/80 dark:bg-[#131317]/50 blur-[50px] z-0 pointer-events-none select-none" />
              
              {/* Responsive skeleton container */}
              <div className="relative w-full max-w-[290px] aspect-[1.12/1] select-none z-10 transition-all duration-500 hover:scale-[1.03]">
                {mounted ? (
                  <Image
                    src={getIllustrationSrc()}
                    alt="Skully Authentication Skeleton Illustration"
                    fill
                    className="object-contain"
                    priority
                  />
                ) : (
                  <div className="w-full h-full bg-muted/40 rounded-3xl animate-pulse" />
                )}
              </div>

              {/* Decorative tags */}
              <div className="mt-8 text-center space-y-1 z-10 max-w-[280px]">
                <h4 className="text-xs font-black tracking-tight text-foreground">
                  {currentView === "login" && "Secure Ledger Encryption"}
                  {currentView === "signup" && "Establish New Necromancy"}
                  {currentView === "forgot" && "Recover Obsidian Access"}
                  {currentView === "check-email" && "Reset Ticket Dispatched"}
                </h4>
                <p className="text-5xs font-semibold text-muted-foreground leading-normal">
                  Your identity is cryptographic. All requests are hashed and sealed in type-safe unions automatically.
                </p>
              </div>
